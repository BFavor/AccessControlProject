...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if ((currentPage != "index.html" && username) && (currentPage != "totp.html" && username)) {
...
document.addEventListener('DOMContentLoaded', () => {
    const pagesRequiringJWT = ['/user_profile.html', '/query.html', '/messaging.html']; 
    const currentPage = window.location.pathname;

    // Only proceed if the current page requires a JWT
    if (!pagesRequiringJWT.some(page => currentPage.includes(page))) {
        console.log("This page does not require JWT, skipping JWT checks."); // Debugging
        return;
    }
    ...
    console.log("JWT found, executing profile-related code."); // Debugging
    ...
    async function fetchProfileData() {
        ...
        try {
            const response = await fetch('/api/profile', {
                headers: { 'Authorization': JWT }  // Adjusted to JWT only
            });
            ...
            if (data.profile_picture) {
                ...
                document.getElementById('profile-picture').src = '/uploads/default-picture.jpeg';
            }            
            document.getElementById('username').innerText = data.username;
            document.getElementById('bio').innerText = data.bio || 'No bio provided.';
            document.getElementById('algae-facts').innerText = data.algae_facts || '';
            if (data.profile_picture) {
                document.getElementById('profile-picture').src = data.profile_picture;
            }
    
            // Populate the textareas in edit mode
            document.getElementById('new-bio').value = data.bio || '';
            document.getElementById('new-facts').value = data.algae_facts || '';
    ...
    async function saveProfileChanges() {
        ...
        const newFacts = document.getElementById('new-facts').value;
        const newProfilePicture = document.getElementById('new-profile-picture').files[0];

        const formData = new FormData();
        formData.append('bio', newBio);
        formData.append('algae_facts', newFacts);
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert('Failed to update profile: ' + errData);
            }
        } catch (err) {
            console.error(err.message);
        }
...
function goToUserProfile() {
    location.href = "http://" + parsedUrl.host + "/user_profile.html";
}
